...
for($i=0; $i<count($SrcMS); $i++) {
    ...
    if($j==$CountInt) { $IntMS[$j]=array('MZ'=>$MZ,'INT'=>$INT,'I100'=>0); $IntMS[$j]['MZ']=$MZ; $IntMS[$j]['INT']=$INT; }
}
...
for($j=0; $j<count($IntMS); $j++) $IntMS[$j]['I100']=0.0+$IntMS[$j]['INT']*100/$MaxINT;
...
for($j=0; $j<count($IntMS); $j++) printf("%2ld \t%9.4f \t%7.0f \t%5.1f\n", $j, $IntMS[$j]['MZ'], $IntMS[$j]['INT'], $IntMS[$j]['I100']);
