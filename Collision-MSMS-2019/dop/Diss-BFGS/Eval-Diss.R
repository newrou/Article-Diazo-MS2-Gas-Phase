...
#    print( sprintf(" *** %f = %f - %f (k=%f)", d, A[i], Af[i], k ) )
 ...
 print( sprintf("dG=%.2f eV   B=%.2f   Quality: %.3e [%s]", dG, B , Sum/n, caption), quote = FALSE, justify = "none", row.names = FALSE )
 cap <- sprintf("[%s] dG=%.2f eV  B=%.2f  Quality=%.3e", caption, dG, B, Sum/n )
 main = "Dissociation"
 location = "topright"
...
ControlPar <- list(trace=0, maxit=70000, abstol=1e-12, reltol=1e-12)
